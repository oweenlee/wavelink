...
pub fn analyze_all_replaygain(app: tauri::AppHandle, state: State<AppState>) {
    ...
    std::thread::spawn(move || {
        let to_analyze: Vec<&(String, Option<f64>)> = entries.iter().filter(|(_, g)| g.is_none()).collect();
        batch_process(to_analyze, &app, "replaygain", |(path, _)| {
            let lufs = rg_analyze(&PathBuf::from(path))?;
            let gain = gain_for_loudness(lufs);
            let db = sdk::library::LibraryDb::open(&db_path).map_err(|e| format!("db open: {e}"))?;
            db.set_track_gain(path, gain).map_err(|e| format!("db write: {e}"))
        });
    });
}
