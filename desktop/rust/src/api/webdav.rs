...
/// （keep-alive），避免批处理每首歌重复 TLS/TCP 握手。
/// （std OnceLock 实现，桌面端不引 once_cell 依赖）
...
pub async fn engine_play_webdav_stream(
    ...
    let handle = crate::api::engine::engine_start_stream(format_hint, content_length, seek_secs)?;
