...
fn main() {
    ...
    // 动态链接 libc++_shared（oboe 是 C++ 库，oboe-ext 预编译静态库引用了
    // __cxa_pure_virtual 等 C++ ABI 符号，必须链接 libc++。
    // 用动态而非静态：c++_static 需要额外 -L 指向 NDK sysroot，且依赖链复杂。
    // libc++_shared.so 由移动端 android/app/src/main/jniLibs 随 APK 打包。
    // 注意：cargo:rustc-link-lib 输出不受 cargokit 的 CARGO_ENCODED_RUSTFLAGS 覆盖，
    // 因此任何构建链都能自动链接，无需手动传环境变量。
