...
fn generate_wav_bytes(seconds: u32) -> Vec<u8> {
    ...
    let channels = 2u16;
    // 44 字节 WAV 头 + PCM 数据
    let data_len = sample_rate * seconds * channels as u32 * 2; // 16bit = 2字节
...
fn test_play_stream_returns_handle_immediately() {
    ...
    let result = _handle.play_stream(Some("wav".to_string()), None, None);
...
fn test_play_stream_feeds_data_then_playing() {
    ...
    let sh = handle
        .play_stream(Some("wav".to_string()), None, None)
        .expect("流式启动应成功");
