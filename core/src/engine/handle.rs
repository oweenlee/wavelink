...
impl EngineHandle {
    ...
    pub fn play(&self, path: String) {
        let _ = self.tx.send(EngineCommand::Play(path, None));
    }
    /// 同步播放（等待引擎确认启动成功）
    ...
    pub fn prev_track(&self) {
        let _ = self.tx.send(EngineCommand::PrevTrack);
    }
    /// 暂停播放
    pub fn pause(&self) {
    ...
    pub fn seek(&self, pos: f64) {
        let _ = self.tx.send(EngineCommand::Seek(pos, None));
    }
    /// 同步跳转（等待引擎确认 seek 完成）
    ...
    pub fn load_ir(&self, path: String) {
        let _ = self.tx.send(EngineCommand::LoadIr(path));
    }
    /// 清除脉冲响应（恢复平坦响应）
    pub fn clear_ir(&self) {
        let _ = self.tx.send(EngineCommand::ClearIr);
    }
    /// 设置参数均衡器某频段的参数
    ...
    pub fn set_auto_eq(&self, name: Option<&str>) {
        let _ = self
            .tx
            .send(EngineCommand::SetAutoEq(name.map(|s| s.to_string())));
    }
    /// 设置 DSD 播放模式（下次播放生效）。
    ...
    pub fn set_stereo_widener(&self, enabled: bool, width: f32) {
        let _ = self
            .tx
            .send(EngineCommand::SetStereoWidener { enabled, width });
    }
    /// 设置跨馈
    ...
    pub fn set_play_mode(&self, mode: PlayMode) {
        let _ = self.tx.send(EngineCommand::SetPlayMode(mode));
    }
    /// 从队列中移除指定索引的曲目
    pub fn remove_from_queue(&self, index: usize) {
        let _ = self.tx.send(EngineCommand::RemoveFromQueue(index));
    }
    ...
    pub fn set_output_device_sync(&self, name: String) -> Result<(), EngineError> {
        ...
        let _ = self
            .tx
            .send(EngineCommand::SetOutputDevice(name, Some(ack_tx)));
        ack_rx
            .recv_timeout(Duration::from_secs(5))
            .unwrap_or(Err(EngineError::InvalidState("应答超时".into())))
    }
